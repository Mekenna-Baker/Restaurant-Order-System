def place_order(menu):
    ...
    while True:
        ...
        # Extract the food category and the options for each category already extracted in loop above

            # Loop through the options for each food category

            for meal, price in options.items():

            # Extract the meal and the price for each option meal and price are already extracted from options.items()
        ...
        if continue_ordering == 'n':
            ...
            #  Create an order_total from the prices list using sum(). Round the prices to 2 decimal places.

            order_total = round(sum(prices_list), 2)

            #  Exit the ordering loop, either use a break statement or set the condition to False
...
def update_order(order, menu_selection, menu_items):
    ...
    if menu_selection.isdigit():
        ...
        if menu_selection in menu_items:
            ...
            # Ask the customer for the quantity of the menu item. Use the item name variable in the question
            ...
            #  Add a dictionary to the order list. The dictionary should include the item name, price, and quantity
            # Use the following names for the dictionary keys: "Item name", "Price", "Quantity"
        ...
        #  When the user's input isn't valid, tell the customer that their input isn't valid
            ...
            print('Invalid selection. This item does not exist on the menu.')

    # When the menu selection wasn't valid: Print the menu selection and Tell the customer they didn't select a menu option

    else:
        print(f"'{menu_selection}' is not a valid menu option. Please enter a number from the menu.")
...
def print_itemized_receipt(receipt):
    ...
    for item in receipt:
        ...
        print_receipt_line(item_name, price, quanity)
